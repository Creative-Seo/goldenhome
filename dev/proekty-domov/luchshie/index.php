<?php $title = 'Лучшие проекты красивых домов и коттеджей из газоблоков';?>
<?php $description = 'Лучшие проекты красивых домов и коттеджей из газоблоков от компании Golden Home';?>
<?php $keywords = 'Лучшие проекты домов, лучшие проекты коттеджей, проекты красивых домов, проекта дома под ключ цена, проекты двухэтажных домов, проекты домов из газобетона, проект строительства дома, проект двухэтажного дома';?>
<?php $breadcrumb = 'Лучшие проекты';?>
<?php $breadcrumb2 = 'Проекты домов';?>
<?php $root = '../../';
include($root."blocks/meta.php"); 
include($root."blocks/header.php");?>

<div class="container">

          <h1>Проекты красивых загородных домов</h1>
        
	<div class="row" id="proekts" itemtype="http://schema.org/ItemList" itemscope>
<?php $class_block_proekt = 'col-sm-6 col-md-3 bl-pr';
include("../proekt-58-120/block.php");
include("../proekt-57-212/block.php");
include("../proekt-56-135/block.php");
include("../proekt-55-220/block.php");
include("../proekt-54-160/block.php");
include("../proekt-53-320/block.php");
include("../proekt-52-142/block.php");
include("../proekt-50-110/block.php");
include("../proekt-49-179/block.php"); 
include("../proekt-48-255/block.php");
include("../proekt-47-248/block.php");
include("../proekt-46-161/block.php");
include("../proekt-45-163/block.php");
include("../proekt-44-193/block.php");
include("../proekt-41-125/block.php");
include("../proekt-40-325/block.php");
include("../proekt-39-125/block.php");
include("../proekt-34-177/block.php");?>
    </div>
</div>
<?php include($root."blocks/footer.php");?>